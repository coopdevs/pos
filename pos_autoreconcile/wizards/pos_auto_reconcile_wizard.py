...
class AutoReconcileWizard(models.TransientModel):
    ...
    def reconcile(self):
        ...
        # works but risks crashing on the stack depth limit, use iterator?
        def group_lines(current, todo, grouped):
            """
                returns a dictionary of lines grouped by key
                (date, ref, partner id) and w/ debit and credit balanced.
                Grouped lines are sorted under the key generated by the sequence
                "sequence_temporary_matching_number". Unbalanced lines are under
                the empty key ''.

                note: assumes lines are grouped by key

                -> iterates over the lines in _todo
                -> adds line to current if current's debit and credit are
                    unbalanced
                -> adds current to grouped if balanced
                -> adds unbalanced lines to grouped under empty '' key
            ...
            if len(todo) == 0:
                ...
                return group_lines([head(todo)], tail(todo), grouped)
            elif sum_credit(current) == sum_debit(current):
                # matched group
                grouped[self.next_rn()] = current
                return group_lines([], todo, grouped)
            elif key(head(current)) != key(head(todo)):
                # no match for group
                grouped[""] = grouped.get("", []) + current
                return group_lines([], todo, grouped)
            else:
                return group_lines(current + [head(todo)], tail(todo), grouped)

        grouped = group_lines([], move_lines, {})

        for rn, lines in grouped.items():
            for line in lines:
...
class AutoReconcileWizardLine(models.TransientModel):
    ...
    _order = "reconcile_number,date,ref,partner_id,debit,credit"
